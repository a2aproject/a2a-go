...
func TestMetadataCarrierImplementation(t *testing.T) {
	tests := []struct {
		...
		{"Part (Text)", &Part{Content: Text("hello")}},
		{"Part (Data)", &Part{Content: Data(map[string]any{"foo": "bar"})}},
		{"Part (URL)", &Part{Content: URL("https://cats.com/1.png")}},
		{"Part (Raw)", &Part{Content: Raw([]byte{0xFF, 0xFE})}},
