...
2. **Write is synchronous** -- it blocks until all still-registered readers receive the message (via a `dispatched` channel). Writer connections are publishers only and do not participate in fan-out. A writer admits at most one unfinished broadcast, so returning early on context cancellation cannot build an abandoned backlog; the accepted broadcast remains ordered and is still delivered. This preserves producer-side backpressure for a normally slow reader.
