...
type Database interface {
	// Create persists a new task that contains a single event.
	Create(ctx context.Context, task *a2a.Task, event a2a.Event, newVersion a2a.TaskVersion, protocolVersion a2a.ProtocolVersion) error
	// Update persists an updated task appending a new event to its list.
	// It returns true if the operation was successful (or false if optimistic locking failed).
	Update(ctx context.Context, task *a2a.Task, event a2a.Event, newVersion a2a.TaskVersion, prevVersion a2a.TaskVersion, protocolVersion a2a.ProtocolVersion) (bool, error)
...
func NewDBTaskStore(db Database, version a2a.ProtocolVersion) *DBTaskStore {
	return &DBTaskStore{db: db, protocolVersion: version}
}
...
func (s *DBTaskStore) Save(ctx context.Context, task *a2a.Task, event a2a.Event, prevVersion a2a.TaskVersion) (a2a.TaskVersion, error) {
	newVersion := a2a.TaskVersion(time.Now().UnixNano())

	if prevVersion == a2a.TaskVersionMissing {
		err := s.db.Create(ctx, task, event, newVersion, s.protocolVersion)
		if err != nil {
			return a2a.TaskVersionMissing, err
		}
	} else {
		success, err := s.db.Update(ctx, task, event, newVersion, prevVersion, s.protocolVersion)
		if err != nil {
			return a2a.TaskVersionMissing, err
		}
		if !success {
