...
func TestDefaultRequestHandler_OnSendMessageStreaming_AuthRequired(t *testing.T) {
	...
	executor := &mockAgentExecutor{
		ExecuteFunc: func(ctx context.Context, reqCtx *RequestContext, q eventqueue.Queue) error {
			if err := q.Write(ctx, a2a.NewStatusUpdateEvent(reqCtx.StoredTask, a2a.TaskStateAuthRequired, nil)); err != nil {
				return err
			}
			<-authCredentialsChan
			result := a2a.NewStatusUpdateEvent(reqCtx.StoredTask, a2a.TaskStateCompleted, nil)
			result.Final = true
			return q.Write(ctx, result)
		},
	}
...
func TestDefaultRequestHandler_OnSendMessage_NewTaskHistory(t *testing.T) {
	...
	executor := &mockAgentExecutor{
		ExecuteFunc: func(ctx context.Context, reqCtx *RequestContext, q eventqueue.Queue) error {
			event := a2a.NewStatusUpdateEvent(reqCtx.StoredTask, a2a.TaskStateCompleted, nil)
			event.Final = true
